...
# TRACK: LAB-2
# COMPONENT: ec2
# PURPOSE: Standardized script header for LAB-2.
###############################################################################

set -euo pipefail

# -----------------------------------------------------------------------------
# LAB-2B bootstrap (SSM + app + CloudWatch Logs)
# -----------------------------------------------------------------------------
...
#######################################################################
# Commented out hardcoded region and secret ID. These should 
# be passed via environment variables for flexibility.

# REGION = os.environ.get("AWS_REGION", "ap-northeast-1")
# SECRET_ID = os.environ.get("SECRET_ID", "lab-2a/rds/mysql")

# secrets = boto3.client("secretsmanager", region_name=REGION)
########################################################################
#
# Added IMDS-based region detection as a fallback if env vars aren't set. 
# This allows the app to run without modification in the lab environment, 
# while still supporting more flexible configuration via env vars if desired.
...
def _imds_identity_doc(token: str):
    req = urllib.request.Request(
...
# Prefer explicit env var if present; otherwise derive region from IMDS (works even under systemd)
REGION = os.environ.get("AWS_REGION") or os.environ.get("AWS_DEFAULT_REGION")
if not REGION:
    try:
        token = _imds_token()
        REGION = _imds_identity_doc(token)["region"]
    except Exception:
        # Last-resort fallback to avoid breaking the app: keep your lab-2 region
        REGION = "us-east-1"
...
Environment=SECRET_ID=lab-2a/rds/mysql
