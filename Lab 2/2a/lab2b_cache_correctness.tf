...
resource "aws_cloudfront_origin_request_policy" "chewbacca_orp_static01" {
  ...
  cookies_config {
    cookie_behavior = "none"  # Static content doesn't need cookies
  }
  headers_config {
    header_behavior = "whitelist"
    headers {
      items = [
        "Origin",  # Add Origin for CORS if needed
      ]
    }
  }
  query_strings_config {
    query_string_behavior = "none"  # Static content doesn't need query strings
  }
}
# -------------------------------------------------
