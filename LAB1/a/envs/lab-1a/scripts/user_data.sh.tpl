...
# app.py with Terraform variable interpolation
...
# Dynamic values injected by Terraform
REGION = "${region}"
SECRET_ID = "${secret_id}"
RDS_HOST = "${rds_host}"
...
def get_db_creds():
    ...
    # Inject the current RDS host dynamically
    # Strip port if accidentally included
    host_only = RDS_HOST.split(':')[0]
    s["host"] = host_only
    return s
...
# systemd service with Terraform variable interpolation
