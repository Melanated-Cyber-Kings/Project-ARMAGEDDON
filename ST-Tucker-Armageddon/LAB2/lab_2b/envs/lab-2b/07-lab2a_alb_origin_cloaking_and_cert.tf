...
# HTTP listener (origin path): CloudFront connects here (origin_protocol_policy=http-only)
resource "aws_lb_listener" "http_80" {
  load_balancer_arn = aws_lb.app_alb.arn
  port              = 80
  protocol          = "HTTP"
...
# If header matches -> forward, else stays 403 (default action above)
resource "aws_lb_listener_rule" "allow_only_cloudfront_header_http" {
