...
# CloudFront Distribution with Failover and WAF Integration
...
resource "aws_cloudfront_distribution" "medical_global_dist" {
  ...
  provider = aws.us_east_1 # CloudFront must be created in us-east-1
...
*/
  origin {
    domain_name = "shinjuku-origin.devlab405.click"
...
*/
  origin {
    domain_name = "liberdade-origin.devlab405.click"
  ...
  logging_config { #added for lab 3b
    ...
    bucket          = data.aws_s3_bucket.audit_log_vault.bucket_domain_name
