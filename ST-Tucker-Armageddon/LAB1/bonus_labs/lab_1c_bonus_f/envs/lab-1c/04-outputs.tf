...
output "rds_db_identifier" {
  description = "RDS DB instance identifier for grader gates"
  value       = module.rds.db_identifier
}

############################################
# Secrets Manager (core for grading)
############################################
output "rds_secret_id" {
  description = "Secrets Manager secret id used by the environment."
  value       = module.secrets.secret_id
}

output "rds_secret_arn" {
  description = "Secrets Manager secret ARN used by the environment."
  value       = module.secrets.secret_arn
}

output "rds_secret_name" {
  description = "Secrets Manager secret name used by the environment."
...
output "app_fqdn" {
  description = "App FQDN (e.g., app.example.com)."
...
output "waf_log_destination" {
  value = module.ingress.waf_log_destination
}

output "waf_cw_log_group_name" {
  value = module.ingress.waf_cw_log_group_name
}

# Outputs for WAF logging to S3 via Firehose (if enabled)
# These may be null if WAF logging is disabled or configured to log to CloudWatch instead.

output "waf_logs_s3_bucket" { value = module.ingress.waf_logs_s3_bucket }
output "waf_firehose_name" { value = module.ingress.waf_firehose_name }
output "waf_firehose_dest_bucket" { value = module.ingress.waf_firehose_dest_bucket }
output "waf_web_acl_arn" { value = module.ingress.waf_web_acl_arn }
