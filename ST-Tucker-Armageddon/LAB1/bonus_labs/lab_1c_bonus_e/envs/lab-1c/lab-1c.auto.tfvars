...
# Added option for user to choose between CloudWatch Logs or S3 for WAF log storage.
# If using S3, ensure you have an S3 bucket created and provide the bucket name and prefix below.
# In this lab, S3 bucket is only created if the user selects S3 as the log destination.

# The default is CloudWatch Logs for simplicity, but S3 can be more cost-effective for 
# long-term storage and analysis.

# Realistically the user would not run this lab long enough for log retention costs to be significant, 
# but this is included to demonstrate best practices and options for WAF logging.
# So your three options for WAF log destination are:
#   - cloudwatch : Logs stored in CloudWatch Logs (default)
#   - s3         : Logs stored in S3 bucket (provide bucket name and prefix)
#   - firehose    : Logs sent to Kinesis Firehose for custom processing (not implemented in this lab)
