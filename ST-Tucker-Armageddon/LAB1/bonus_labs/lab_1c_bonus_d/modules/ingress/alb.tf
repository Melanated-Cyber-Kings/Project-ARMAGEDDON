...
resource "aws_lb" "app_alb" {
  ...
  # For Lab-1C Bonus D, we want internet-facing ALB. The default is "internet-facing", so we can omit the scheme or set it explicitly.
  ...
  # For Lab-1C Bonus D, we want internet-facing ALB. Set internal to false for internet-facing, true for internal. 
  ...
  # Online I found that the scheme argument can be an issue depending on the AWS provider version. 
  # In some versions, the "scheme" argument is not supported or has specific requirements.
  ...
  # We add dynamic block for access_logs to conditionally enable ALB access logging based on the variable var.enable_alb_access_logs.
  # If var.enable_alb_access_logs is true, the access_logs block will be included in the ALB configuration, enabling access logging 
  # to the specified S3 bucket and prefix. If var.enable_alb_access_logs is false, the access_logs block will be omitted, and 
  # access logging will not be enabled for the ALB.
  # This approach allows us to easily toggle ALB access logging on or off without having to modify the ALB resource directly,
  # making our Terraform configuration more flexible and maintainable.

  # Note: When enabling ALB access logs, ensure that the specified S3 bucket exists and has the appropriate permissions for the ALB to write logs. 
  # The bucket should have a policy that allows the ALB to put objects into the bucket, and the prefix can be used to organize logs within the bucket.
  # Reference: https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/lb#access_logs
