...
Through the core lab and five advanced bonuses, this project evolved from a standard 3-tier monolith into a Hardened, Zero-Trust, Self-Observing Infrastructure. I didn't just build a web server; I built a secure delivery pipeline that mirrors the architecture of top-tier cloud organizations.
## Key Takeaways
By leveraging IAM roles and VPC Endpoints, I made sure that even if a server is compromised, it only has access to exactly what it needs to function.The entire environment, from the DNS records to the WAF logging destination, is defined in Terraform. This ensures 100% repeatability and eliminates "Configuration Drift. A system is only as reliable as its monitoring. By linking WAF logs, ALB logs, and CloudWatch alarms, we created a "glass house" where every failure is visible and actionable.