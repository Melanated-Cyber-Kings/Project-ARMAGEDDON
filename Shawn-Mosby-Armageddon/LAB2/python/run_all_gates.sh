...
if [[ "$REQUIRE_WAF_ASSOCIATION" == "true" ]]; then
  if [[ -n "$cf_resource_arn" ]]; then
    ...
    if [[ -n "$waf_assoc_arn" && "$waf_assoc_arn" != "None" ]]; then
      add_detail "PASS: WAF WebACL is associated with CloudFront (Manually Verified)."
...
if [[ -n "$LOG_BUCKET" ]]; then
  ...
  if [[ "$logging_enabled" == "true" ]]; then
    # Use a simpler wildcard match to bypass the trailing dot/hidden char issue
    if [[ "$logging_bucket" == *"$LOG_BUCKET"* ]]; then
