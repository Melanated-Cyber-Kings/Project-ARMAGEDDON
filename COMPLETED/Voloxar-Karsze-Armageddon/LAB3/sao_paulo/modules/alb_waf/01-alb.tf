...
resource "aws_lb" "alb02" {
  name               = "${var.project}-alb02"
...
resource "aws_lb_listener" "http_listener02" {
  load_balancer_arn = aws_lb.alb02.arn
  port              = 80
  protocol          = "HTTP"
...
resource "aws_lb_listener" "https_listener02" {
  load_balancer_arn = aws_lb.alb02.arn
  port              = 443
  protocol          = "HTTPS"
...
resource "aws_lb_listener_rule" "require_origin_header02" {
  listener_arn = aws_lb_listener.https_listener02.arn
  priority     = 10
...
resource "aws_lb_listener_rule" "default_block02" {
  listener_arn = aws_lb_listener.https_listener02.arn
  priority     = 99
