...
resource "aws_cloudwatch_log_group" "waf_log_group02" {
  ...
  name              = "aws-waf-logs-${var.env_prefix}-webacl02"
  retention_in_days = var.waf_log_retention_days

  tags = {
    Name = "${var.env_prefix}-waf-log-group02"
  }
}

# Explanation: This wire connects the shield generator to the black box—WAF -> CloudWatch Logs.
resource "aws_wafv2_web_acl_logging_configuration" "waf_logging02" {
  count = var.enable_waf && var.waf_log_destination == "cloudwatch" ? 1 : 0

  resource_arn = aws_wafv2_web_acl.waf02[0].arn
  depends_on = [aws_wafv2_web_acl.waf02]
  
  log_destination_configs = [
    aws_cloudwatch_log_group.waf_log_group02[0].arn
  ]
