...
resource "aws_cloudfront_distribution" "ccf02" {
  enabled         = true
  is_ipv6_enabled = true
  comment         = "${var.project}-cf02"

  origin {
    origin_id   = "${var.project}-alb-origin02"
    ...
    custom_header {
      ...
      value = random_password.origin_header_value02.result
    }
  }
  ...
  default_cache_behavior {   # <--- your block goes here
    target_origin_id       = "${var.project}-alb-origin02"
    viewer_protocol_policy = "redirect-to-https"
    ...
    origin_request_policy_id = aws_cloudfront_origin_request_policy.orp_api02.id
  }
 ...
 ordered_cache_behavior {
  ...
  cache_policy_id         = aws_cloudfront_cache_policy.cache_static02.id
  origin_request_policy_id = aws_cloudfront_origin_request_policy.orp_static02.id
  viewer_protocol_policy  = "redirect-to-https"
 ...
 # Explanation: Attach WAF at the edge — now WAF moved to CloudFront.
  ...
  logging_config {
    bucket = "${aws_s3_bucket.cf_logs2.bucket}.s3.amazonaws.com"
    prefix = "cloudfront/"
...
resource "random_password" "origin_header_value02" {
  length  = 32
  special = false
}
