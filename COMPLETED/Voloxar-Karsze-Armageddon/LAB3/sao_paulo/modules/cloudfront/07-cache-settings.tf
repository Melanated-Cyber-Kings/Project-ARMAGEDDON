...
# from Google Gemini
resource "aws_cloudfront_cache_policy" "cache_api_disabled02" {
  name        = "${var.project}-cache-api-disabled02"
  default_ttl = 0
  max_ttl     = 0
  min_ttl     = 0
...
#------------------------------------------
resource "aws_cloudfront_origin_request_policy" "orp_api02" {
  name = "${var.project}-orp-api02"
...
resource "aws_cloudfront_response_headers_policy" "rsp_static02" {
  name    = "${var.project}-rsp-static02"
...
resource "aws_cloudfront_cache_policy" "cache_static02" {
  name        = "${var.project}-cache-static02"
...
resource "aws_cloudfront_origin_request_policy" "orp_static02" {
  name    = "${var.project}-orp-static02"
  comment = "Minimal forwarding for static assets"
