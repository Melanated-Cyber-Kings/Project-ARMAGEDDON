...
resource "aws_cloudfront_distribution" "cf" {
  ...
  ###########################################################################
  # Default behavior (conservative): treat as dynamic unless path matched below
  # - Safe default: caching disabled, forward API-relevant context to origin
  ###########################################################################
  default_cache_behavior {
    target_origin_id       = "${local.name_prefix}-alb-origin01"
    viewer_protocol_policy = "redirect-to-https"
  ...
  ###########################################################################
  # /api/* behavior (explicit): caching disabled (safe default)
  ###########################################################################
  ...
  ###########################################################################
  # /static/* behavior: aggressive caching + explicit Cache-Control header
  ###########################################################################
  ordered_cache_behavior {
    path_pattern           = "/static/*"
    target_origin_id       = "${local.name_prefix}-alb-origin01"
  ...
  ###########################################################################
  # Attach WAF at the edge (CLOUDFRONT scope Web ACL)
  ###########################################################################
