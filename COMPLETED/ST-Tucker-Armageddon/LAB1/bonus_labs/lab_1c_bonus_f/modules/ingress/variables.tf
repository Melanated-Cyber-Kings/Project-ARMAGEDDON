...
variable "domain_name" {
  type        = string
  description = "Base domain for DNS + ACM (example: example.com)."
}
...
variable "enable_waf_sampled_requests_only" {
  description = "Placeholder toggle (future filtering/redaction)."
  type        = bool
  default     = false
}
...
###############################################################################
# WAF Logs teardown safety
###############################################################################

# Added this to allow safe teardown of WAF logging buckets. 
# Bad stuff happens when these buckets are deleted accidentally.

# For this lab, we will keep this true to allow cleanup. In production, 
# you would typically set this to false to prevent accidental deletion of critical log data.

variable "waf_logs_force_destroy" {
  description = "If true, allow Terraform to delete WAF logging buckets (lab teardown safety)."
  type        = bool
  default     = true
}
