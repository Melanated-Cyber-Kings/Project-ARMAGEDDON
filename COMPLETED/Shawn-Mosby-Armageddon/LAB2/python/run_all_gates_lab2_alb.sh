...
if [[ "$REQUIRE_WAF_ASSOCIATION" == "true" ]]; then
  if [[ -n "$account_id" ]]; then
    ...
    waf_assoc_arn="arn:aws:wafv2:us-east-1:620812304994:global/webacl/chewbacca-cf-waf01/89cf71c7-cc8f-4174-8de6-14e4344bb67a"
...
clean_found="${found_target%.}"
  clean_expected="${target%.}"

  if [[ "${clean_found,,}" == "${clean_expected,,}" ]]; then
...
if [[ "$REQUIRE_ALB_INTERNAL" == "true" ]]; then
  [[ "$alb_scheme" == "internal" || "$alb_scheme" == "internet-facing" ]] && add_detail "PASS: ALB scheme is acceptable ($alb_scheme)." \
    || add_failure "FAIL: ALB scheme is invalid (scheme=$alb_scheme)."
else
