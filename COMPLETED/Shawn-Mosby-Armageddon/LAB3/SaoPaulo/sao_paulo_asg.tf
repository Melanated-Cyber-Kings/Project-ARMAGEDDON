...
resource "aws_launch_template" "liberdade_lt" {
  ...
  iam_instance_profile { #needed for ssm for CLI checks
    name = data.terraform_remote_state.tokyo.outputs.ssm_profile_name
  }
  ...
  user_data = base64encode(<<-EOF
              ...
              echo "DB_ENDPOINT=${data.terraform_remote_state.tokyo.outputs.rds_endpoint}" > /root/db_config.txt
