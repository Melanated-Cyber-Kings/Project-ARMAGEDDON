...
# 1. New rule for HTTPS (Front Door - Secure)
resource "aws_vpc_security_group_ingress_rule" "ec2_https" {
  description       = "Allow HTTPS traffic for Bonus-B"
  security_group_id = aws_security_group.chewbacca_ec2_sg01.id
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 443
  to_port           = 443
...
# 2. New rule for Internal Traffic (ALB to EC2)
resource "aws_vpc_security_group_ingress_rule" "ec2_internal_alb" {
  description                  = "Allow ALB to talk to EC2"
  security_group_id            = aws_security_group.chewbacca_ec2_sg01.id
...
resource "aws_secretsmanager_secret" "chewbacca_db_secret01" {
  name = "${local.name_prefix}/lab-1a/rds/mysql"
}
...
# The Interface Endpoints (SSM, Secrets, Logs)
...
# S3 Gateway Endpoint (Crucial for DNF/YUM in private subnets)
