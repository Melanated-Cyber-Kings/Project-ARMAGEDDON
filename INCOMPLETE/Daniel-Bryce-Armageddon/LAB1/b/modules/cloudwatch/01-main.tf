...
resource "aws_sns_topic_subscription" "email" {
  ...
  protocol  = "email"
  endpoint  = "innerp670@gmail.com" # ⚠️ CHANGE THIS
}

# 2. CloudWatch Log Group (if not already exists)
resource "aws_cloudwatch_log_group" "app_logs1" {
  name              = "/aws/ec2/lab-rds-app1"
  retention_in_days = 7
}
...
resource "aws_cloudwatch_metric_alarm" "db_failure" {
  ...
  dimensions = {
    LogGroupName = aws_cloudwatch_log_group.app_logs1.name
  }
...
resource "aws_cloudwatch_log_metric_filter" "db_errors" {
  ...
  pattern        = "CRITICAL"
  log_group_name = aws_cloudwatch_log_group.app_logs1.name
